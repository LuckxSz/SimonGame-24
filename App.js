...
//         get a random color from colors obj
...
//        pause execution of game for given amount of time
...
const generateRandomPath = async () => {
  ...
  isPathGenerating.length;
  isPathGenerating = true;
  await showPath(randomColors);
};

//        show the path of colors to player
...
const showPath = async (colors) => {
  ...
  for (let color of colors) {
    ...
    // pause execution for 500 millisecs
    await delay(500);
...
const handleColorClick = async (e) => {
  ...
  //        if clicked color is correctm update score and continue generating the path
